...
func (r *JigasiReconciler) SetupWithManager(mgr ctrl.Manager) error {
...
func (r *JigasiReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	reqLogger := r.Log.WithValues("name", req.Name, "namespace", req.Namespace)
	jigasi, err := jitsi.NewJigasi(ctx, r.Client, reqLogger, req)
	if err != nil {
		if meeterr.IsUnderDeletion(err) {
			if delErr := jigasi.Delete(); delErr != nil {
	...
	if createErr := jigasi.Create(); createErr != nil {
		if apierrors.IsAlreadyExists(createErr) {
			if updErr := jigasi.Update(); updErr != nil {
